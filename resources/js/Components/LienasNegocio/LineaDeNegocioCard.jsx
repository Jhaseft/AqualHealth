export default function LineaDeNegocioCard ({ titulo, imagen, servicios, imagenfondo, bg }) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-6 md:mb-8 h-auto">
    
      <div 
        className="w-full md:w-60 lg:w-80 flex-shrink-0 p-2 md:p-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300"
        style={{ backgroundImage: `linear-gradient(to right, ${bg.from}, ${bg.to})` }}
      >
        <div className="w-full h-48 sm:h-56 md:h-full rounded overflow-hidden">
          <img 
            src={imagen} 
            alt={titulo}
            className="w-full h-full object-contain rounded-3xl border-8 border-white"
          />
        </div>
      </div>
      ...
      >
        ...
        <div className="hidden lg:block absolute -right-24 xl:-right-32 top-0 bottom-0 w-60 xl:w-80 z-10">
